...
| `group_guid`   | string (UUID) | Условно | GUID группы. **Обязателен** (вместе с `group_name` и `faculty_name`), если в БД ещё нет группы с данным `group_oid` — тогда сервер создаёт запись группы. Если группа уже есть, поле можно не передавать |
| `group_name`   | string  | Условно | Название группы. То же условие, что у `group_guid` |
| `faculty_name` | string  | Условно | Название факультета. То же условие, что у `group_guid` |

Если для указанного `group_oid` группы нет, без непустых `group_guid`, `group_name` и `faculty_name` запрос завершится ответом **400** (см. ниже). Существующая группа при этом **не** обновляется из тела запроса.
...
Создание группы по `group_oid`, но не переданы или переданы как `null` обязательные для этого поля (`group_guid` / `group_name` / `faculty_name`):
...
Если в теле передан новый ненулевой `group_oid`, для которого в БД ещё нет группы, действуют те же **условно обязательные** поля `group_guid`, `group_name`, `faculty_name`, что и при `POST` (иначе **400** с теми же текстами `detail`).
