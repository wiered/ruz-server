...
- [Регистрация активности пользователя](#регистрация-активности-пользователя)
...
### PUT `/api/user/{user_id}/touch`

Фиксирует текущее время активности пользователя в поле `last_used_at`. Эндпоинт
следует вызывать при взаимодействии пользователя с клиентским приложением. Тело
...
| --- | --- | --- |
| `user_id` | integer | ID пользователя, для которого регистрируется активность. |
...
curl -X PUT \
  ...
  http://localhost:8000/api/user/123456/touch
```

**Response 200** — время активности успешно обновлено
...
**Response 500** — не удалось сохранить новое время активности
