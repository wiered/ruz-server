...
def _seed_user_and_lessons(
    engine, *, user_group_oid: int = 7001, user_subgroup: int = 1
):
    with Session(engine) as session:
        ...
        auditorium = Auditorium(
            id=6001, guid=uuid.uuid4(), name="A-101", building="Main"
        )
        ...
        session.add_all(
            [
                LessonGroup(lesson_id=lesson.id, group_id=user_group_oid)
...
class TestScheduleAPI:
    ...
    async def test_day_returns_only_date_and_sorted(self, client):
        ...
        response = await client.get(
            "/api/schedule/user/123456/day", params={"date": "2025-01-13"}
        )
    ...
    async def test_week_returns_lessons_in_week_range(self, client):
        _seed_user_and_lessons(client.engine)
        response = await client.get(
            "/api/schedule/user/123456/week", params={"date": "2025-01-14"}
        )
        assert response.status_code == 200
        data = response.json()
    ...
    async def test_user_without_group_returns_400(self, client):
        with Session(client.engine) as session:
            session.add(
                User(id=200001, username="no_group_user", group_oid=None, subgroup=1)
            )
            session.commit()
        response = await client.get(
            "/api/schedule/user/200001/day", params={"date": "2025-01-13"}
        )
    ...
    async def test_user_not_found_returns_404(self, client):
        response = await client.get(
            "/api/schedule/user/999999/day", params={"date": "2025-01-13"}
        )
    ...
    async def test_empty_result_returns_empty_list(self, client):
        _seed_user_and_lessons(client.engine)
        response = await client.get(
            "/api/schedule/user/123456/day", params={"date": "2025-02-01"}
        )
        assert response.status_code == 200
    ...
    async def test_week_boundary_excludes_next_week_lessons(self, client):
        _seed_user_and_lessons(client.engine)
        response = await client.get(
            "/api/schedule/user/123456/week", params={"date": "2025-01-19"}
        )
        assert response.status_code == 200
        lesson_ids = [row["lesson_id"] for row in response.json()]
        assert 1005 not in lesson_ids
